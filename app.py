...
        </p>
    </div>
""".replace("Upload a CSV or Excel file to create a ranking chart", "Create a ranking chart from your uploaded data.").replace("Last updated 25/10/25 – ", ""), unsafe_allow_html=True)
# *** END MODIFICATIONS ***
...
    )
    # Store uploaded file in session state to check for its presence later
...
    # *** MODIFICATION 1: Moved this message earlier, only keep the rest of the content here for initial load ***
    # st.info("⬆️ **Please upload your data file using the controls in the sidebar (Section 1) to begin chart configuration.**")
    st.markdown("---")
    
...
    2.  **Filter (Optional):** Apply data filters to focus on specific subsets (Section 5)
...
    5.  **Customize:** Adjust order and set display count (Section 6)
    6.  **Title:** Set your chart title (Section 2)
